...
namespace WhistleWindLobotomyMod
{
    ...
    {
        private NewSpecialAbility HiddenEvolveHelper()
        {
            const string rulebookName = "SpecialEvolve";
            const string rulebookDescription = "You shouldn't see this";
            return WstlUtils.CreateSpecialAbility<SpecialEvolve>(
                AbilitiesUtil.LoadAbilityIcon("None"),
                rulebookName, rulebookDescription, overrideDesc: true);
        }
    }
    public class SpecialEvolve : SpecialCardBehaviour
    {
        ...
        public static SpecialAbilityIdentifier GetSpecialAbilityId
        {
            get
            {
                return SpecialAbilityIdentifier.GetID(WhistleWindLobotomyMod.Plugin.pluginGUID, "SpecialEvolve");
            }
        }
        ...
        public override IEnumerator OnUpkeep(bool playerUpkeep)
        {
            ...
            {
                yield return new WaitForSeconds(0.25f);
                CardInfo evolutionGreed = CardLoader.GetCardByName("wstl_kingOfGreed");

                foreach (CardModificationInfo item in base.Card.Info.Mods.FindAll((CardModificationInfo x) => !x.nonCopyable))
                {
                    CardModificationInfo cardModificationInfo = (CardModificationInfo)item.Clone();
                    evolutionGreed.Mods.Add(cardModificationInfo);
                }
                yield return base.PlayableCard.TransformIntoCard(evolutionGreed);
                yield return new WaitForSeconds(0.5f);
            ...
            {
                yield return new WaitForSeconds(0.25f);
                CardInfo evolutionTrue = CardLoader.GetCardByName("wstl_nothingThereEgg");
                foreach (CardModificationInfo item in base.Card.Info.Mods.FindAll((CardModificationInfo x) => !x.nonCopyable))
                {
                    CardModificationInfo cardModificationInfo = (CardModificationInfo)item.Clone();
                    evolutionTrue.Mods.Add(cardModificationInfo);
                }
                yield return base.PlayableCard.TransformIntoCard(evolutionTrue);
                yield return new WaitForSeconds(0.25f);
            ...
            {
                yield return new WaitForSeconds(0.25f);
                CardInfo evolutionEgg = CardLoader.GetCardByName("wstl_nothingThereFinal");

                foreach (CardModificationInfo item in base.Card.Info.Mods.FindAll((CardModificationInfo x) => !x.nonCopyable))
                {
                    CardModificationInfo cardModificationInfo = (CardModificationInfo)item.Clone();
                    evolutionEgg.Mods.Add(cardModificationInfo);
                }
                yield return base.PlayableCard.TransformIntoCard(evolutionEgg);
                yield return new WaitForSeconds(0.25f);
