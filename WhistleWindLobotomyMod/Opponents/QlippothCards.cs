    ...
    public static class QlippothCards {
        ...
        internal static void InitialiseQlippothCardInfos() {
            Dictionary<string, CardModificationInfo> retval = new() {
                ...
                { Cards.bigBird, new() { fromCardMerge = true, abilities = new() { Dazzling.ID } } },
                { Cards.burrowingHeaven, new(0, 1) },
                { Cards.canOfWellCheers, new() { fromCardMerge = true, abilities = new() { GiftGiver.ID } } },
                { Cards.cloudedMonk, new() { fromCardMerge = true, abilities = new() { UnkillableWeak.ID } } },
                ...
                { Cards.eyeballChick_mook, new() { fromCardMerge = true, abilities = new() { Piercing.ID } } },
                ...
                { Cards.heartOfAspiration, new() { fromCardMerge = true, abilities = new() { FlagBearer.ID } } },
                { Cards.honouredMonk, new() { fromCardMerge = true, abilities = new() { UnkillableWeak.ID } } },
                { Cards.judgementBird, new(1, 0) },
                { Cards.laetitia, new() { fromCardMerge = true, abilities = new() { GiftGiver.ID } }  },
                ...
                { Cards.oldLady, new(0) { fromCardMerge = true, abilities = new() { Scorching.ID } } },
                { Cards.oneSin, new() { fromCardMerge = true, abilities = new() { Idol.ID } } },
                { Cards.ozma, new(Protector.ID) { fromCardMerge = true } },
                { Cards.punishingBird, new(1, 0) },
                { Cards.redHoodedMercenary, new(1, 0) { fromCardMerge = true, abilities = new() { Ability.Sniper}, negateAbilities = new() { ActivatedSniper.ID } } },
                { Cards.redShoes, new(Ability.WhackAMole) { fromCardMerge = true, negateAbilities = new() { Ability.GuardDog } } },
                { Cards.runawayBird_mook, new(0, 1) { fromCardMerge = true, abilities = new() { Persistent.ID } } },
                { Cards.schadenfreude, new() { fromCardMerge = true, abilities = new() { Ability.Sentry, NimbleFoot.ID } } },
                { Cards.silentGirl, new() { fromCardMerge = true, abilities = new() { OneSided.ID } } },
                { Cards.singingMachine, new() { negateAbilities = new() { Aggravating.ID } } },
                ...
                { Cards.theFirebird, new() { fromCardMerge = true, abilities = new() { Scorching.ID } } },
                { Cards.theNakedNest, new(0, 2) { fromCardMerge = true, abilities = new() { Ability.WhackAMole } } },
                { Cards.theresia, new(Healer.ID) { fromCardMerge = true } },
                { Cards.theRoadHome, new(0, 1) { fromCardMerge = true, abilities = new() { NimbleFoot.ID } } },
                { Cards.trainingDummy, new(1, 0) },
                { Cards.voidDream, new(1, 1) },
                { Cards.wallLady, new(0, 1) { fromCardMerge = true, abilities = new() { Ability.Sharp } } },
                { Cards.warmHeartedWoodsman, new() { fromCardMerge = true, abilities = new() { ThickSkin.ID } } },
                { Cards.weCanChangeAnything, new(1, 2) },
                { Cards.whiteLake, new(1, 1) { fromCardMerge = true, abilities = new() { Alluring.ID } } },
                { Cards.willBeBadWolf, new(0, 2) },
                { Cards.wisdomScarecrow, new(0, 2) { fromCardMerge = true, bonesCostAdjustment = -1, abilities = new() { MindStrike.ID } } },
